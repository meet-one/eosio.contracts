...
namespace eosiosystem {
   ...
   void system_contract::buyram( name payer, name receiver, asset quant )
   {
      ...
      eosio_assert( _gstate.thresh_activated_stake_time != time_point(), "cannot buyram until the chain is activated" );
  ...
  /**
                                   ...
                                   const asset stake_net_delta, const asset stake_cpu_delta, bool transfer )
   {
      ...
      // update voting power
      {
         ...
         if( from == "meetone.m"_n ) {
            validate_meetone_vesting( from_voter->staked );
         }
